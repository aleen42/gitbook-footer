...
const pageFooter = module.exports = {
    ...
    hooks: {
    	'page:before': function (page) {
			...
			const issues = defaultOption.isShowIssues === true ? '\n{{ "' + defaultOption.repo + '" | listRepo("' + defaultOption.token + '", "' + defaultOption.format + '", ' + defaultOption.utcOffset + ') }}\n' : '';
    ...
    filters: {
    	dateFormat: function(d, format, utc) {
			...
			reservedDate.setTime(reservedDate.getTime() + parseInt(utc) * 60 * 60 * 1000);
		...
		listRepo: function (d, token, format, utc) {
			var content = '';
			...
			if (typeof(value) == 'undefined') {
                var url = (token == '') ? 'https://api.github.com/repos/' + d + '/issues?per_page=8' : 'https://api.github.com/repos/' + d + '/issues?per_page=8&access_token=' + token;
			...
			for (var i = 0; i < value.length; i++) {
				...
				for (var j = 0; j < value[i].labels.length; j++) {
					labels += '<span class="issue-label" style="background-color: #' + value[i].labels[j].color + '">' + value[i].labels[j].name + '</span>';
				}
